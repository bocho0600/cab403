...
bool create_shared_object( shared_memory_t* shm, const char* share_name ) {
    ...
    // INSERT SOLUTION HERE
    shm->fd = shm_open(share_name, O_CREAT | O_RDWR, 0666); // create shared memory object if the shm name does not exist, open it if it does
    if (shm->fd == -1) {
        fprintf(stderr, "Error creating shared memory object\n");
        shm->data = NULL; //setting shm->data to NULL if creation failed
        return false; //returning false if creation failed
    }
    ...
    if(ftruncate(shm->fd, sizeof(shared_data_t)) == -1){
        shm->data = NULL; //setting shm->data to NULL if ftruncate failed
        return false; //returning false if ftruncate failed
    }
    ...
    if(shm->data == MAP_FAILED){
        return false; //returning false if mapping failed
    }
...
void destroy_shared_object( shared_memory_t* shm ) {
    ...
    // INSERT SOLUTION HERE
    munmap(shm->data, sizeof(shared_data_t)); //removing the mapped memory from the address space (unmap), shm->data is the address of the shared data block
    shm_unlink(shm->name); //removing the shared memory object
    shm->fd = -1; //setting shm->fd to -1
    shm->data = NULL; //setting shm->data to NULL
}
...
double request_work( shared_memory_t* shm, operation_t op, double lhs, double rhs ) {
    ...
    shm->data->operation = op; //setting the operation field of the shared data object to op
    shm->data->lhs = lhs;  //Using 2 arrows because shm is a pointer to a struct, so we need to dereference it first
    shm->data->rhs = rhs;
...
bool get_shared_object( shared_memory_t* shm, const char* share_name ) {
    ...
    // INSERT SOLUTION HERE
    shm->fd = shm_open(share_name, O_RDWR, 0666); //open the shared memory object
    if(shm->fd == -1){
        shm->data = NULL; //setting shm->data to NULL if the operation fails
        return false; //returning false if the operation fails
    }
    // Otherwise, attempt to map the shared memory via mmap, and save the address
    // in shm->data. If mapping fails, return false.
    // INSERT SOLUTION HERE
...
bool do_work( shared_memory_t* shm ) {
    ...
    if(shm->data->operation == op_quit){
        munmap(shm->data, sizeof(shared_data_t)); //removing the shared data object from the address space
        shm->fd = -1; //setting shm->fd to -1
        shm->data = NULL; //setting shm->data to NULL
        retVal = false; //setting retVal to false
    } else {
