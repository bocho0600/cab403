...
void *producer_function(void *arg)
{
    ...
    while (mqueue->keep_running)
    {
        ...
        fflush(stdout);
        sleep(sleeptime); // Pause for a random time
        ...
        mqueue->buffer[mqueue->position++] = message;   // Load message to buffer      
...
void *consumer_function(void *arg)
{
    ...
    while (mqueue->keep_running)
    {
        ...
        message = mqueue->buffer[--mqueue->position]; // Get message from buffer and decrement position
        mqueue->is_room = TRUE;
...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i < producer_number; i++) {
		...
		if (pthread_create(&producer, NULL, producer_function, &messages)) //Error generating thread returns non-zero value
		{
		    printf("error creating thread.");
		    abort();
		}
	}
    printf("\n %d amount of Producer thread created! \n", producer_number);
    ...
    printf("\n %d amount of Consumer thread created! \n", consumer_number);
